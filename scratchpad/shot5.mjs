...
try {
  await up();
  const su = await fetch(BASE+'/auth/signup',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email:'plus@miles.ai',password:'password123',name:'Plus'})});
  ...
  await fetch(BASE+'/api/profile',{method:'PUT',headers:H,body:JSON.stringify({businessName:'Painters In Philly',industry:'Home Services'})});
  const b = await chromium.launch({ executablePath:'/opt/pw-browsers/chromium-1194/chrome-linux/chrome', headless:true });
  const ctx = await b.newContext({ viewport:{width:1000,height:760}, deviceScaleFactor:2 });
  ...
  await page.click('[data-ask]'); await sleep(500);
  await page.click('#cplus'); await sleep(400);
  await page.screenshot({ path:'/home/user/ai-employee/scratchpad/plusmenu.png', fullPage:true });
